...
export default class CardMain extends Component{
  ...
  render(){
    ...
    let t_index=false;
    return(
      <Ripple style={styles.div} activeOpacity={0.7}
      rippleContainerBorderRadius={27}
      onPress={() => {
          if(t_index==false){
            t_index=true;
        ...
        <Text style={{...styles.header_id}}>{`• ${this.props.item.id}`}<Text style={{...styles.header_time}}>{`   ${rc}`}</Text></Text>          
        {
