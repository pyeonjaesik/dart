...
export default class CardReply4 extends Component{
  constructor(props){
    super(props);
    this.state={
  ...
  render(){
    return(
      <View style={styles.container}>
        {
          this.props.cp==this.props.item.key?(
            <LinearGradient start={{x: 0, y: 0}} end={{x: 0.9, y: 0}} colors={['#e4002b','#fe5000']} 
              style={styles.subject_cp}>
              <Text style={styles.subject_txt_cp}>채택된 게시물</Text>
              <View style={styles.header_cp}>
                <Image source={require('../../assets/top2.png')} style={{width:43,height:43,marginLeft:15}}/>
            ...
            <View style={styles.header}>
              {
                this.props.item.ln>=this.props.lnindex?(
              ...
              <Text style={styles.header_id}>{`${this.props.item.id}`}</Text>
              {
                this.props.mine===true && this.props.item.ln>=this.props.lnindex && this.props.select===false?(
                  <TouchableOpacity style={styles.select} onPress={async ()=>{
                    ...
                    <Text style={styles.select_txt}>채택</Text>
                  </TouchableOpacity>
                ):(null)
              }
            </View>
