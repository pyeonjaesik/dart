...
export default class Alarm2 extends Component{
  ...
  render(){
    return(
      <Ripple style={{...styles.container,backgroundColor:this.state.clicked===false?'#ffffff':'rgba(180,180,180,0.1)'}} activeOpacity={0.7} 
        onPress={async () => {
          if(this.t_index==false){
            this.t_index=true;
            this.props.navigation.push('Mga',{
              post_id:this.props.item.post_id,
              ms_id:this.props.item.ms_id,
              type:4,
            });
          }
        ...
        <View style={styles.div}>
          <View style={styles.imgcontainer}>
            <Image source={require('../../assets/alarm4.png')} style={{width:40,height:40}}/>
          </View>
          <View style={styles.main}>
            <Text style={styles.main_subject}>{'게시물에 새로운 좋아요.'}</Text>
            <Text style={styles.main_subject_v}>{this.props.item.ln+'개의 좋아요가 달렸습니다.'}</Text>
            {
