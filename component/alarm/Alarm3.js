...
export default class Alarm2 extends Component{
  ...
  render(){
    var subject;  
    var subject_v;
    if(this.props.item.ms_id.length>0 && this.props.item.ln>0){
      subject='미션에 새로운 게시물과 좋아요.';
      subject_v=this.props.item.ms_id.length+'개의 게시물과\n'+this.props.item.ln+'개의 좋아요가 달렸습니다.';
    }else if(this.props.item.ms_id.length>0 && this.props.item.ln==0){
      subject='미션에 새로운 게시물.';
      subject_v=this.props.item.ms_id.length+'개의 게시물이 달렸습니다.'
    }else if(this.props.item.ms_id.length==0 && this.props.item.ln>0){
      subject='미션에 새로운 좋아요.';
      subject_v=this.props.item.ln+'개의 좋아요가 달렸습니다.'
    }
    return(
      <Ripple style={{...styles.container,backgroundColor:this.state.clicked===false?'#ffffff':'rgba(180,180,180,0.1)'}} activeOpacity={0.7} onPress={() => {
        ...
        <View style={styles.div}>
          <View style={styles.imgcontainer}>
            <Image source={require('../../assets/alarm3.png')} style={{width:40,height:40}}/>
          </View>
          <View style={styles.main}>
            <Text style={styles.main_subject}>{subject}</Text>
            <Text style={styles.main_subject_v}>{subject_v}</Text>
            {
