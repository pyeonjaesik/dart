...
class HomeSide1 extends React.Component {
  ...
  async componentWillReceiveProps(nextProps) {
    if(nextProps.home1refresh !== this.state.home1refresh) {
      await this.setState({ home1refresh: nextProps.home1refresh });
      this._onRefresh();
    }
  }
  changHeader(index){
  ...
  render() {
    return (
      <FlatList maxToRenderPerBatch={20} style={{backgroundColor:'rgba(255,255,255,0)',flex:1}}
        ...
        renderItem={({item}) => <CardMain  item={item} navigation={this.props.navigation}/>}
        onEndReached={()=>{
...
export default connect(mapStateToProps,mapDispatchToProps)(HomeSide1);