...
import CardReply4 from './card/CardReply4';
...
class MgaScreen extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      refreshing: false,
  ...
  getfetch =()=>{
    if(this.props.navigation.getParam('type')===4){
      var data={
        _id:this.props.navigation.getParam('post_id'),
        user_id:this.props._id,
        ms_id:this.props.navigation.getParam('ms_id',0)
      };
    }else{
      var data={
        _id:this.props.navigation.getParam('post_id'),
    ...
    let mgtype=this.props.navigation.getParam('type');
    let mgurl;
    switch(mgtype){
      case 1:
        mgurl=`/oneposta1`;
    ...
    .then((responseJson) => {
      if(responseJson.status===100){
        ...
        console.log(responseJson.lnindex);
        let post_l=responseJson.post.length;
        this.post_arr=responseJson.post;
        if(post_l<=20){
          this.p_index=0;
      ...
      case 4:
        return (
          <CardReply4 
            _id={this.props._id} 
            item={item}
            cp={this.state.post[0].cp} 
            onDialog={this.onDialog}
            onDialog2={this.onDialog2}
            mine={this.state.mine} 
            select={this.state.select}
            setIDMS={this.setIDMS}
            lnindex={this.state.lnindex}
          />
        )      
    }
  }
...
export default connect(mapStateToProps,mapDispatchToProps)(MgaScreen);